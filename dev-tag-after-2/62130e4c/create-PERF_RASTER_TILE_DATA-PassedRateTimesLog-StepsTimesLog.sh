...
perftestname=('output-PERF_RASTER_TILE_DATA_FITS') # 'PERF_CONTOUR_DATA' 'PERF_CUBE_HISTOGRAM' 'PERF_REGION_SPECTRAL_PROFILE' 'PERF_ANIMATION_PLAYBACK')
...
#    echo $TestResults
 ...
 Step21=`cat $n* | grep "$img" | grep "RasterTileData" | sed "s/(Step 2)/ /g" | sed "s/$img/ /g"  `
...
#    read time21 <<<${tempStep12//[^0-9]/ }
 ...
 if [[ $tempStep21 == *"Go to "* ]]; then
  echo $date $time $n $img '2' '3000' '3000'>>$perfname-Step2TimesLog.txt
 else
...
perftestname=('output-PERF_RASTER_TILE_DATA_CASA') # 'PERF_CONTOUR_DATA' 'PERF_CUBE_HISTOGRAM' 'PERF_REGION_SPECTRAL_PROFILE' 'PERF_ANIMATION_PLAYBACK')
# lenperftest="${#perftestname[@]}"
...
perftestname=('output-PERF_RASTER_TILE_DATA_HDF5') # 'PERF_CONTOUR_DATA' 'PERF_CUBE_HISTOGRAM' 'PERF_REGION_SPECTRAL_PROFILE' 'PERF_ANIMATION_PLAYBACK')
# lenperftest="${#perftestname[@]}"
...
cat output-PERF_RASTER_TILE_DATA_FITS-PassedRateTimesLog.txt output-PERF_RASTER_TILE_DATA_CASA-PassedRateTimesLog.txt output-PERF_RASTER_TILE_DATA_HDF5-PassedRateTimesLog.txt > output-PERF_RASTER_TILE_DATA-PassedRateTimesLog.txt
cat output-PERF_RASTER_TILE_DATA_FITS-Step1TimesLog.txt output-PERF_RASTER_TILE_DATA_CASA-Step1TimesLog.txt output-PERF_RASTER_TILE_DATA_HDF5-Step1TimesLog.txt > output-PERF_RASTER_TILE_DATA-Step1TimesLog.txt
cat output-PERF_RASTER_TILE_DATA_FITS-Step2TimesLog.txt output-PERF_RASTER_TILE_DATA_CASA-Step2TimesLog.txt output-PERF_RASTER_TILE_DATA_HDF5-Step2TimesLog.txt > output-PERF_RASTER_TILE_DATA-Step2TimesLog.txt
exit
